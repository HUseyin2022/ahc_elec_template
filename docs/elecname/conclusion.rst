...
Dolev-Klawe-Rodeh (DKR) Algorithm
    ...
    Conclusion
    Secondly, DKR exhibits a message complexity of O(NlogN), where N represents the number of nodes in the system. This complexity implies that as the size of the distributed system grows, the number of messages exchanged during the leader election process increases logarithmically, which is advantageous for scalability.
