...
Complexity:  HS Algorithm algorithm also works in ring topologies and has a message complexity of  O(nlogn), which is generally better than DKR when M is significantly larger than n.
...
DKR exhibits a message complexity of O(NlogN), where N represents the number of nodes in the system. This complexity implies that as the size of the distributed system grows, the number of messages exchanged during the leader election process increases logarithmically, which is advantageous for scalability.
...
DKR guarantees termination and correctness under various network conditions, making it suitable for practical deployment in real-world distributed systems
